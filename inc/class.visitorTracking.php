...
class visitorTracking
{
    ...
    */
	private function db_connect()
	{
		...
		try
		{
			$this->link = new mysqli( 'localhost', 'root', 'admin', 'corva' );
	...
	protected function track()
	{
		...
		$visitor_cname		= trim($visitor_country[0]);
	...
	private function getFlag($countryCode)
	{
		...
		$flag = '<img src="src/files/countryflags/' . strtolower($countryCode) . '.gif" height="15px" width="25px"/>';
	...
	*/
	...
	/**
	 * Get number of visits from our website
	 *
	 * This method queries the database for all entries in the visitors table, 
	 * then counts the number of total visits.
	 *
	 * @param null
	 * @return number of visits, HTML
	 */
	public function getNumberOfVisits(){
		...
		if( $this->link->error )
		{
			...
			{
				echo $r['count'];
				// returns total count, number of visits
			}
		}
	}
